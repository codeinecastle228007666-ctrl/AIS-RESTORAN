// Форма управления заказами (список, поиск, смена статуса, оплата)
using _1.data;
using _1.forms;
...
namespace _1
{
    // Главная форма списка заказов: создание, поиск, смена статуса, оплата, просмотр состава.
    // Статусы заказов: 1-Новый, 2-Принят, 3-Готовится, 4-Готов, 5-Оплачен, 6-Завершён, 7-Отменён
    public partial class Zakazi : Form
    {
        ...
        // Загрузка заказов с фильтром по поиску, скрытие служебных ID.
        // SQL: динамическое добавление WHERE — строковая сборка, но значение @search
        // передаётся параметром (не конкатенация), поэтому SQL-инъекция невозможна.
        ...
        // Переход к оплате выбранного заказа (доступно для статуса "Готов").
        ...
        // SQL UPDATE: оба параметра (@status, @id) — через NpgsqlParameter, безопасно.
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            string newStatusName = comboBox1.Text;

            if (MessageBox.Show($"Изменить статус заказа #{zakazId} на \"{newStatusName}\"?",
