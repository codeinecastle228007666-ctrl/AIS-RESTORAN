...
{
    // Форма бронирования столов: создание, удаление, смена статуса, просмотр броней.
    // Статусы брони: 1-Новая, 2-Подтверждена, 3-Отменена, 4-Завершена, 5-Неявка
    public partial class BronirovanieForm : Form
    {
        ...
        // Загрузка списка броней в DataGridView.
        void LoadBron()
        {
            string sql = @"
                SELECT
        ...
        // Создание новой брони с проверкой доступности и вместимости.
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Проверка: вместимость стола для количества гостей
                int capacity = GetTableCapacity(stol, tr);
                if (guests > capacity)
                {
                    tr.Rollback();
                    MessageBox.Show($"Данный стол рассчитан максимум на {capacity} гостей.");
                    return;
                }

                // Проверка: не занят ли стол в это же время (бронь и активные заказы)
                if (TableIsBusy(stol, date, tr))
                {
                    tr.Rollback();
                    MessageBox.Show("Данный стол уже занят на указанное время.");
                    return;
                }

                // INSERT с параметрами — все значения (@c, @s, @d, @g, @st) передаются отдельно от SQL.
                string sql = @"
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Ошибка удаления бронирования:\n" + ex.Message);
                return;
            }
        ...
        // Проверка, занят ли стол в указанное время (учитывает брони и активные заказы в окне 2 часа).
        // Все параметры (@stol, @start, @end) — через AddWithValue, SQL-инъекция невозможна.
        bool TableIsBusy(int stol, DateTime date, NpgsqlTransaction tr)
        {
            // Окно проверки: 2 часа
            DateTime end = date.AddHours(2);

            // Проверка броней с активным статусом (не отменён/завершён/неявка)
            string sql = @"
                SELECT COUNT(*)
                FROM bronirovanie
                WHERE stol_id = @stol
        ...
        // Визуальный выбор стола из зала.
        ...
        // Применение выбранного статуса к бронированию.
        private void buttonApplyStatus_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Недопустимая смена статуса.");
                return;
            }

            if (MessageBox.Show($"Изменить статус бронирования #{bronId}?", "Подтверждение",
                MessageBoxButtons.YesNo) == DialogResult.No)
                return;
        ...
        // Получение текущего статуса бронирования.
        ...
        private bool IsTransitionAllowed(int oldStatus, int newStatus)
        {
            ...
            // Конечные статусы: отмена/завершено/неявка
            if (oldStatus == 3 || oldStatus == 4 || oldStatus == 5)
                return false;

            // Нельзя вернуться в статус "Новая"
        ...
        // Обновление состояния UI в зависимости от текущего статуса.
        private void UpdateStatusUI()
        {
            ...
            if (currentStatusId == 3 || currentStatusId == 4 || currentStatusId == 5)
            {
                ...
                buttonApplyStatus.Text = "Статус изменён";
                return;
            }
            ...
            if (currentStatusId == 1)
                ...
                buttonApplyStatus.Text = "Завершить / Отменить";
        }
        ...
        // Цветовая индикация статуса бронирования.
