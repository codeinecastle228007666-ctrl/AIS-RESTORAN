...
namespace _1.forms
{
    // Форма для создания или редактирования сотрудника с валидацией полей.
    // Поля: ФИО (textBoxFio), должность (comboBoxDolzhnost), телефон, email, серия/номер паспорта, адрес, дата рождения
    public partial class SotrudnikEditForm : Form
    {
        private int _sotrudnikId = -1; // -1 = новый сотрудник
        ...
        // Подключение валидации ввода к текстовым полям.
        ...
        private void LoadData()
        {
            ...
            if (row["dolzhnost_id"] != DBNull.Value)
            ...
            // Поле даты может быть DateOnly или DateTime в зависимости от драйвера PostgreSQL
        ...
        // Сохранение сотрудника (добавление или обновление в БД).
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (_sotrudnikId == -1)
                {
                    // Добавление нового сотрудника (restoran_id = 1 по умолчанию)
                    string sql = @"
        ...
        // Валидация ФИО: только буквы, пробелы, тире, точки.
        ...
        // Валидация ввода: только цифры (для серии паспорта).
        ...
        // Проверка корректности email через MailAddress.
